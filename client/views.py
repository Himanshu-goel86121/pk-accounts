...
def add_client(request):
    try:
        ig = client(client_name=request.POST['client_name'].strip(),
                    under_bank_accounts=request.POST['under_bank_accounts'],
                    address=request.POST['address'].strip(), city=request.POST['city'].strip(),
                    state=request.POST['state'].strip(), pincode=request.POST['pincode'].strip(),
                    phone1=request.POST['phone1'].strip(), phone2=request.POST['phone2'].strip(),
                    gstin=request.POST['gstin'].strip(), pan_no=request.POST['pan_no'].strip(),
                    nickname=request.POST['nickname'].strip(), email=request.POST['email'].strip())
        ig.save()
        return render(request, 'client_add.html', {
...
def modify_client(request):
    ...
    if (check_user.role != 'Admin'):
        return render(request, 'client_modify.html',
                      {"clients": clients, 'error_message': "You dont have the permission to modify anything", })
    try:
        try:
            selected_choice = client.objects.get(pk=request.POST['client_name'])
        except (KeyError, client.DoesNotExist):
            return render(request, 'client_modify.html',
                          {"clients": clients, 'error_message': "The item group name provided has not been added", })
        ...
        return render(request, 'client_modify.html',
                      {"clients": clients, 'success_message': "Item Group saved successfully", })
    except Exception as e:
        print(e)
        print(e.__class__.__name__)
        if (str(e.__class__.__name__) == 'DataError'):
            return render(request, 'client_modify.html',
                          {"clients": clients, 'error_message': "Please provide the data correctly", })
        if (str(e.__class__.__name__) == 'ValidationError'):
            return render(request, 'client_modify.html',
                          {"clients": clients, 'error_message': "Please provide the data correctly", })
        else:
...
def delete_client(request):
    ...
    try:
        ...
        return render(request, 'client_delete.html',
                      {"clients": clients, 'success_message': "Item Group deleted successfully", })
    except Exception as e:
        print(e)
        raise
        print(e.__class__.__name__)
